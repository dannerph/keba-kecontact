...
class Wallbox(ABC):
    ...
    def __init__(self, keba, device_info: WallboxDeviceInfo, loop=None):
        ...
        # Internal variables
        self._refresh_interval = MAX_POLLING_INTERVAL
    ...
    async def set_failsafe(self, timeout=30, fallback_value=6, persist=0):
        ...
        await self._send('failsafe ' + str(timeout) + ' ' + str(fallback_value * 1000) + ' ' + str(persist), fast_polling=True)
    ...
    async def display(self, text: str, mintime: int =2, maxtime: int=10):
        ...
        await self._send("display 1 " + str(int(round(mintime))) + ' ' + str(int(round(maxtime))) + " 0 " + text[0:23])
